...
required_pkgs <- c("dplyr", "tidyr", "ggplot2", "lubridate")
...
# optional packages (don't fail the build if missing)
has_DT     <- requireNamespace("DT", quietly = TRUE)
has_marked <- requireNamespace("marked", quietly = TRUE)
...
`PITmodelR` streamlines access to [PTAGIS](https://www.ptagis.org/) data through API endpoints and provides tools for PIT-tag data workflows, including project discovery, mark–recapture–recovery (MRR) data retrievals, site metadata, tag observations, and survival/migration modeling. 

The package does not replace the user's responsibility to ensure PTAGIS data are used properly and subset to a sample of tags representing their group or population of interest. Additionally, the modeling tools within can lead to erroneous results if careful thought isn't given at each step and if mark-recapture model assumptions are violated.

This vignette demonstrates how to:

* Install the package
* Configure API authentication
* Retrieve mark-recapture-recovery project codes, including available years of data for each code
* Explore MRR and interrogation site metadata
* Download MRR and PIT-tag observation data
* Retrieve tag event histories

> **Note:** Code chunks below requiring network calls or an API key are set to `eval = FALSE` to keep this vignette runnable. Try them locally after setting up your API key.
...
# install PITmodelR (faster, safer version)
...
If `PITmodelR` was installed using `build_vignettes = TRUE`, vignettes can be accessed using:
...
Some [PTAGIS endpoints](https://api.ptagis.org/) require an API key, which is a unique string that identifies you as an authorized user i.e., a password that allows the `PITmodelR` package to access data on your behalf. Many of the functions in `PITmodelR` require an API key to successfully reach out to these PTAGIS endpoints. You can request a key from PTAGIS staff at this [link](https://www.ptagis.org/Contact). You can then pass it directly to functions as a character string using the argument `api_key` (when needed), or set it once for your R session as an environmental variable: 
...
Individual mark-recapture-recovery (MRR) files submitted to PTAGIS by a given project and year can be retrieved with `get_mrr_files()`. The function returns a small data frame (i.e., tibble) containing metadata about the submitted files: 
...
After identifying which files are available, we can retrieve all submitted data from a specific file using `get_file_data()`.
By default, this function returns a structured list containing:

* `session` – information about the MRR session.
* `events` – each individual mark, recapture, or recovery event recorded during the session.
* `session_pdv_fields` – metadata describing session-level project-defined variables (if present).
* `detail_pdv_fields` – metadata describing event-level project-defined variables (if present).
* `pdv_values` – extracted PDV values stored in long format.

```{r eval = FALSE}
# Johnson Creek screw trap file from March 22, 2024
...
# view each object in mrr_data
mrr_data$session
mrr_data$events
mrr_data$session_pdv_fields
mrr_data$detail_pdv_fields
mrr_data$pdv_values$session
mrr_data$pdv_values$events

```

If the user is not interested in retaining project-defined variables (PDV) metadata and values, `drop_pdvs = TRUE` can be set, in which case only `session` and `events` will be returned.

```{r eval = FALSE}
# drop pdv metadata and values
mrr_no_pdvs <- get_file_data("CDR-2024-082-JCT.xml", drop_pdvs = TRUE)

```

> **Note:** PDVs are not automattically attached to the `session` and `events` at this stage. Attaching and mapping PDVs is handled at the batch retrieval level using `get_batch_file_data()` desribed below.

## Batch Retrieve MRR Data

Multiple MRR files can be retrieved from PTAGIS and combined using `get_batch_file_data()`. The function returns a list with:

* `sessions` - one row per MRR file/session.
* `events` - all MRR event records combined across files.
* `pdv_map` — (only when `pdvs = "attach"`) a mapping table describing project-defined variables.

By default, `get_batch_file_data()` will only return `sessions` and `events`. However, the argument `pdvs = "attach"` can easily be used to retain session-level and event-level PDVs in which case `pdv_map` will also be returned providing metadata and mapping information for PDVs.

If `pdvs = "attach"` is used, then argument `map_pdvs` can be set to either `FALSE` or `TRUE`. If `map_pdvs = FALSE`, PDVs are attached using standardized, coded column names e.g., `spdv1`, `spdv2` for session-level PDVs and `pdv1`, `pdv2` for event-level PDVs. If `map_pdvs = TRUE`, PDVs are mapped to their descriptive label-based columns.

Here we can explore these behaviors by compiling all data submitted by project "CDR" during 2024 from the Secesh River rotary screw trap:

```{r eval = FALSE}
# get file names containing "SCT" (Secesh Trap)
secesh_rst_files <- files$name[grepl("SCT", files$name)]

# default behavior, returns just sessions and events
all_data <- get_batch_file_data(filenames = secesh_rst_files)

# retain pdvs, map to coded column names
all_data <- get_batch_file_data(filenames = secesh_rst_files,
                                pdvs = "attach",
                                map_pdvs = FALSE)

# retain pdvs, map to labeled column names
all_data <- get_batch_file_data(filenames = secesh_rst_files,
                                pdvs = "attach",
                                map_pdvs = TRUE)

# explore items in all_data
all_data$sessions
all_data$events
all_data$pdv_map    # if pdvs = "attach"

```

> **Note:** Project-defined variables (PDVs) are optional and may (often) vary across files. Because user defined fields may contain dates, numbers, character strings, etc., all data in those fields are returned as a character string. Additional data formatting may be necessary after if user defined fields require a specific format.

```{r echo = FALSE}
load(system.file("extdata/vignette_data/all_data.rda",
                 package = "PITmodelR",
                 mustWork = T))

```

## Joining Session and Event Data

At this point, the `sessions` and `events` can easily be joined into a single data frame, if desired, using the `file_name` column.

```{r eval = TRUE}
# join events to sessions
events_df <- all_data$sessions |>
  left_join(all_data$events, by = "file_name")
...
table(events_df$event_type, events_df$species_run_rear_type)
...
Survival studies begin with a list of tags (i.e., marks) that represent the group or population of interest that can later be recaptured or observed. The above functions allow us to retrieve data associated with single or multiple MRR files (in this case, mark or tagging files) submitted to PTAGIS. To create a list of tags suitable for a survival study, we first need to clean the MRR data and filter for tags that best represent our group of interest. This can be completed using functions from the `tidyverse` [packages](https://tidyverse.org/).

## Mark List

Creating mark groups requires careful thought and consideration. For instance, you might want to compare survival (and detection probabilities) between parr and smolts from a single location, or between smolts released at two or more locations, or you might want to look at differences between multiple taggers of a hatchery release group. Either way, carefully consider the research or management question at hand before defining your mark group of interest. Defining and creating mark groups is a critical first step in any survival study and will directly impact your analysis and interpretation of results.
...
mark_group <- events_df |>
  dplyr::filter(
    species_run_rear_type == "12W",              # wild summer chinook
    migration_year == 2025,                      # emigrating in 2025
    dplyr::between(release_date, 
                   lubridate::ymd(20240901), 
                   lubridate::ymd(20241231)),    # fall trapping period
    release_site == "SECTRP",                    # Secesh RST
    pit_tag != "..........",                     # only records with pit tags (i.e., exclude tallies)
    !grepl("Recapture", event_type),             # exclude recaptures
...
We have our marks, now we need all the observations (i.e., detections) that represent the migration history of each fish. For a single PIT tag, use the `get_tag_history()` function; for multiple tags, use `get_batch_tag_histories()`. Both of these functions require the user to include a PTAGIS API key using the argument `api_key`. The mark record and all subsequent observations will eventually be used to create capture histories, which are then used in survival and detection models.
...
tag_history <- get_tag_history(tag_code = mark_group$pit_tag[1])

# list of tags
tag_list <- mark_group$pit_tag
...
my_sites = obs_sites |>
  filter(site_code %in% unique(tag_history$site_code))
```
...
tail(my_sites) |>
  dplyr::select(site_code, name, site_type, rkm)
...
Next, we define the occasions (group of sites that will produce a single parameter) and then assign individual sites to each. In this case, we will estimate survivals from SECTRP to ZEN, SFG, and Lower Granite Dam (GRJ and GRS); all locations downstream of Lower Granite Dam (LGR) can be used to calculate a detection efficiency (and survival) to LGR.

```{r eval = TRUE}
# assign individual detection sites to each model occasion
locs <- list(
  SECTRP = "SECTRP",
  ZEN    = "ZEN",
  SFG    = "SFG",
...
ch_list <- build_mark_histories(
  ...
  site_col     = "site_code",
  tag_col      = "tag_code",
  time_col     = "event_date",  # optional but helps order ties/revisits
  enforce_order = TRUE
)
...
Notice that no sites were dropped in `ch_list$dropped_summary`, because we included all detection sites in `locs`. However, you could choose to ignore some detection sites and the `build_mark_histories()` function will drop those observations if `keep_unkown = FALSE`.
...
`PITmodelR` includes the `fit_marked_cjs()` function which calls `marked::crm()` (capture-recapture model) and returns tidy results for survival (`Phi`) and detection (`p`) parameters. By default, it uses the `model = "CJS"` option from `crm()`.

In this example, both the survival and detection parameters are modeled as functions of `time`. Here, `time` represents encounter occasion for the grouped `locs`. Specifying `~ time` allows survival and detection probabilities to vary among occasions, rather than assuming they are constant across the locations and reaches. We are not including additional covariates (e.g., fish size, tagging date, flow conditions), so the model assumes that differences in survival and detection arise only from temporal and/or spatial variation represented by `time`.

```{r eval = requireNamespace("marked", quietly = TRUE), warning = FALSE, message = FALSE}
# load marked library
#library(marked)
...
```{r eval = requireNamespace("marked", quietly = TRUE)}
print(fit$plots$phi)
```

For our survival model with assigned occasions the intervals represent:
