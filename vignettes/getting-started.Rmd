...
`PITmodelR` streamlines access to [PTAGIS](https://www.ptagis.org/) data through API endpoints and provides helper tools for PIT-tag data workflows, including project discovery, mark–recapture–recovery (MRR) data retrievals, site metadata, tag observations, and survival/migration modeling. MRR data are returned in standardized `sessions` and `events` tables, with optional support for project-defined variables (PDVs).
...
PDVs are not automatically attached to the data at this stage. Attaching and mapping PDVs is handled at the batch retrieval level using `get_batch_file_data()`. If the user is not interested in retaining PDV metadata and values, `drop_pdvs = TRUE` can be set and only `session` and `events` will be returned.
...
# default, view each object in mrr_data
mrr_data$session
mrr_data$events
...
mrr_no_pdv <- get_file_data("CDR-2024-082-JCT.xml", drop_pdvs = TRUE)

```

## Batch Retrieve MRR Data

Multiple MRR files can be downloaded and combined using `get_batch_file_data()`. The function returns a list with:

* **sessions** - one row per MRR file/session.
* **events** - all event records combined across files.
...
Project-defined variables (PDVs) are optional and may vary across files. When requested, PDVs are attached using standardized code columns (e.g., `spdv1`, `pdv1`) and can optionally be mapped to descriptive label-based columns.
...
all_data <- get_batch_file_data(
  ...
  pdvs = "attach",        # should we keep PDV metadata and data?
  map_pdvs = TRUE         # map PDVs to labeled columns?
)
...
Each MRR file corresponds to one session. As a result, `file_name` provides a reliable key for joining session information onto event records.

```{r eval = FALSE}
events_df <- all_data$events |>
  left_join(all_data$sessions, by = "file_name")
```
...
We have our marks - now we need all the observations (i.e., detections) that represent the migration history of each fish. For a single PIT tag, use the `get_tag_history()` function; for multiple tags, use `get_batch_tag_histories()`. Both of these functions require the user to include a PTAGIS API with the function call. The mark record and all subsequent observations will eventually be used to create capture histories, which are then used in survival and detection models.
...
For survival model and assigned occasions the intervals represent:
